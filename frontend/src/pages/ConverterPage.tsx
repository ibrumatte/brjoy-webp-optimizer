...
export function ConverterPage({
  ...
  return (
    <div className="grid h-full grid-cols-[minmax(0,1fr)_360px] gap-5 p-6">
      ...
      <aside className="side-scroll min-h-0 overflow-auto rounded-2xl border border-border bg-surface-card p-5 shadow-soft">
        <h3 className="mb-4 text-lg font-semibold text-text-primary">{t(locale, 'labels.settings')}</h3>

        <div className="space-y-4">
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-text-secondary">Preset</label>
            <select
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-text-secondary">{t(locale, 'labels.format')}</label>
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => onConfigChange({ formato: 'webp' })}
                className={`rounded-xl border px-3 py-2 text-sm font-semibold ${config.formato === 'webp' ? 'border-brand-blue500 bg-brand-blue500 text-white' : 'border-border bg-white text-text-primary'}`}
              >WebP</button>
              <button
                onClick={() => onConfigChange({ formato: 'png' })}
                className={`rounded-xl border px-3 py-2 text-sm font-semibold ${config.formato === 'png' ? 'border-brand-blue500 bg-brand-blue500 text-white' : 'border-border bg-white text-text-primary'}`}
              >PNG</button>
            </div>
          </div>

          <div>
            <label className="mb-2 block text-xs font-semibold uppercase tracking-wide text-text-secondary">{t(locale, 'labels.quality')} ({config.qualidade}%)</label>
            <input
              type="range"
              min={60}
              max={100}
          ...
          <label className="flex items-center justify-between rounded-xl border border-border bg-slate-50 px-3 py-2 text-sm text-text-primary">
          ...
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              className="rounded-xl border border-border bg-white px-3 py-2 text-sm disabled:bg-slate-100"
              placeholder="Width"
            />
            <input
              type="number"
          ...
          <input
            ...
            className="w-full rounded-xl border border-border bg-white px-3 py-2 text-sm disabled:bg-slate-100"
          />
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-text-secondary">{t(locale, 'labels.brightness')} ({config.brightness}%)</label>
            <input
              type="range"
              min={50}
              max={150}
          ...
          <label className="flex items-center justify-between rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-text-secondary">{t(locale, 'labels.outputFolder')}</label>
            <div className="flex gap-2">
              <input value={config.output_folder} readOnly className="w-full rounded-xl border border-border bg-slate-100 px-3 py-2 text-xs text-text-secondary" />
              <button
                onClick={onPickOutputFolder}
                disabled={config.manter_estrutura || config.substituir_no_lugar}
                className="rounded-xl border border-border bg-white px-3 py-2 text-sm font-semibold text-text-primary disabled:opacity-50"
              >
